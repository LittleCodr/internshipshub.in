...
export function generateMetadata({
  ...
  if (match && match.frontmatter.type === "job" && params.segments.length === 1) {
    return buildJobMetadata(match);
  }
  const keyword = params.segments.join(" ").replace(/-/g, " ");
  return buildListingMetadata({
    title: `Jobs for ${keyword}`,
    description: `Associate and fresher jobs mapped to ${keyword}. Filter further by city, company type, and salary range.`,
    path: `/jobs/${params.segments.join("/")}`,
    keywords: [keyword, "jobs in india", "graduate jobs"]
  });
}

const decodeSegments = (segments: string[]) => segments.map((segment) => segment.replace(/-/g, " ")).join(" ");

export default async function JobDetailOrCategory({
  params,
  searchParams
}: {
  params: { segments: string[] };
  ...
  if (match && match.frontmatter.type === "job" && segments.length === 1) {
      ...
      <article className="flex flex-col gap-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          <div className="flex flex-col gap-6">
            ...
            <PillList title="Preferred majors" items={match.frontmatter.branchesAllowed} />
            <PillList title="Experience" items={[match.frontmatter.experienceRequired]} />
        ...
        {related.length > 0 && (
          <section className="flex flex-col gap-4">
            <h2 className="text-xl font-semibold text-slate-900">Similar jobs</h2>
    ...
    <div className="flex flex-col gap-10">
      <header className="flex flex-col gap-2">
        ...
        <p className="max-w-2xl text-sm text-slate-600">
          Deep-linkable collection ready for programmatic SEO. Layer additional filters to create tightly themed cohorts.
        </p>
      </header>

      <FilterBar options={options} selected={filters} action={`/jobs/${segments.join("/")}`} total={filtered.length} />
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        {!paginated.items.length && (
          <p className="text-sm text-slate-500">
            We are tracking this job theme. Subscribe via RSS soon.
          </p>
        )}
      </div>
