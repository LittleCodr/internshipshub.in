...
export function generateMetadata({
  ...
  if (match && match.frontmatter.type === "research" && params.segments.length === 1) {
    return buildJobMetadata(match);
  }
  const keyword = params.segments.join(" ").replace(/-/g, " ");
  return buildListingMetadata({
    title: `Research internships for ${keyword}`,
    description: `Research internships focused on ${keyword}. Tune filters to find stipend-backed projects with publication potential.`,
    path: `/research/${params.segments.join("/")}`,
    keywords: [keyword, "research internships", "india research roles"]
  });
}

const decodeSegments = (segments: string[]) => segments.map((segment) => segment.replace(/-/g, " ")).join(" ");

export default async function ResearchDetailOrCategory({
  params,
  searchParams
}: {
  params: { segments: string[] };
  ...
  if (match && match.frontmatter.type === "research" && segments.length === 1) {
      ...
      <article className="flex flex-col gap-10">
        ...
        {related.length > 0 && (
          <section className="flex flex-col gap-4">
            <h2 className="text-xl font-semibold text-slate-900">Similar research roles</h2>
  ...
  return (
    <div className="flex flex-col gap-10">
      <header className="flex flex-col gap-2">
        <h1 className="text-3xl font-semibold tracking-tight text-slate-900">
          Research internships tagged “{keyword}”
        </h1>
        <p className="max-w-2xl text-sm text-slate-600">
          Structured programmatic page ready for future filters, citations, and Jobs crawler coverage.
        </p>
      </header>

      <FilterBar options={options} selected={filters} action={`/research/${segments.join("/")}`} total={filtered.length} />
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        {!paginated.items.length && (
          <p className="text-sm text-slate-500">
            We will publish matching research internships soon.
          </p>
        )}
      </div>
      ...
      <Pagination
        ...
        basePath={`/research/${segments.join("/")}`}
