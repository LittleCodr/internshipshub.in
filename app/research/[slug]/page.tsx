...
export async function generateMetadata({
  ...
  const job = await getJobBySlug("research", params.slug).catch(() => null);
  if (!job) {
    return {
      title: "Research internship not found",
...
export default async function ResearchDetailPage({
  ...
  const relatedPool = await getJobsByCategory("research");
        ...
        <div className="space-y-8">
          ...
          {related.length > 0 ? (
            <section className="space-y-4">
              <h2 className="text-xl font-semibold text-slate-900">Related research internships</h2>
