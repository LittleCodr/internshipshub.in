...
export async function generateStaticParams() {
  ...
  return internships.map((job) => ({ slug: job.slug }));
}
...
export async function generateMetadata({
  ...
  const job = await getJobBySlug("internship", params.slug).catch(() => null);
  if (!job) {
    return {
...
export default async function InternshipDetailPage({
  ...
  const relatedPool = await getJobsByCategory("internship");
        ...
        <div className="space-y-8">
          ...
          {related.length > 0 ? (
            <section className="space-y-4">
              <h2 className="text-xl font-semibold text-slate-900">Related internships</h2>
